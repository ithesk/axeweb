...
const MainPage: React.FC<{ data: RepairOrderData; setShowInvoice: (show: boolean) => void; setShowAuthorization: (show: boolean) => void }> = ({ data, setShowInvoice, setShowAuthorization }) => {
  ...
  const deviceFunctions = [
    { name: 'Face ID', status: Boolean(data.faceid), icon: <Smartphone size={20} /> },
    { name: 'Cargado', status: Boolean(data.charging), icon: <Plug size={20} /> },
    { name: 'Camara Frontal', status: Boolean(data.camerafront), icon: <Focus size={20} /> },
    { name: 'Wi-Fi', status: Boolean(data.wifi), icon: <Wifi size={20} /> },
    { name: 'Señal', status: Boolean(data.signal), icon: <Radio size={20} /> },
    { name: 'Cámara', status: Boolean(data.camera), icon: <Camera size={20} /> },
    { name: 'Pantalla', status: Boolean(data.screen), icon: <Smartphone size={20} /> },
    { name: 'Touch', status: Boolean(data.touch), icon: <TouchpadIcon size={20} /> },
    { name: 'True Tone', status: Boolean(data.truetone), icon: <Sun size={20} /> },
    { name: 'Micrófono', status: Boolean(data.microphone), icon: <Mic size={20} /> },
